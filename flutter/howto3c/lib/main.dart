...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<(Uint8List?,String?,String?)> _callAPI(Uint8List img,String question) async {
    ...
    if(response.statusCode==200){
      ...
      if(reply['content']['operation']=='other'||reply['content']['operation']=='done')return (null,reply['content']['thought'] as String,reply['content']['operation'] as String);
      final paintedImg=await drawPointOnImage(img, double.parse(reply['content']['x']), double.parse(reply['content']['y']));
      return (paintedImg,reply['content']['thought'] as String,reply['content']['operation'] as String);
    }else{
      Fluttertoast.showToast(
        msg: "伺服器錯誤${response.statusCode}", // 訊息內容
        toastLength: Toast.LENGTH_LONG, // 顯示時間長度
  ...
  /*Future<void> _startBackground() async {
    try{
      await service.configure(
        ...
        androidConfiguration: AndroidConfiguration(onStart:onStart,isForegroundMode: true,foregroundServiceTypes: [AndroidForegroundType.mediaProjection])
      );
  ...
  /*Future<void> _getDisplayMedia() async {
    if(!_backgroundStart)await _startBackground();
    try {
  ...
  Future<void> _sendImage() async {
    ...
    /*if(_localStream==null){
      ...
      Uint8List img=(await _localStream!.getVideoTracks().first.captureFrame()).asUint8List();
      setState(() {
  ...
  Future<void> _sendMessage() async {
    if (_controller.text.isNotEmpty||_questionNow.isNotEmpty) {
      ...
      final (replyImg,replyText,replyOp) = await _callAPI(_messages.last.imgBytes!, _questionNow);
      setState((){
        _loading=false;
        ...
        if(replyImg!=null){
          ...
          _messages.add(ChatMessage(text: replyOp=='done'?'目標完成無圖片':'其他操作無圖片', isMe: false));
        }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (_loading)
            SpinKitPouringHourGlassRefined(color: Colors.deepPurple,),
        ],
      ),
    );
  }
