...
除此之外，一些重要的注意事項，請嚴格遵守：
- 運行本服務的網站名為HowTo3C，但禁止回覆「您已在HowTo3C網站的文字輸入框中輸入「...」，點擊「送出」按鈕，讓系統處理您的問題。」等與服務本身之操作相關的回覆，請直接回覆使用者該如何操作電腦、手機來完成其目標。
...
- 若在畫面中存取到畫面中有明顯的「HowTo3C」字樣，並且畫面中有聊天室的介面、包含「下一步」、「繼續」、「縮小」、「放大」等按鈕，這是本服務HowTo3C與使用者互動的介面，不要將其當成搜尋引擎或通訊軟體，不要使用這些按鈕或輸入框來引導使用者，也不要對這些元素進行click或write，優先使用click、write指令告訴使用者回到主頁面或在網頁中開啟新分頁。只有在明確在畫面中間定位到Google搜尋，或者在畫面上方出現瀏覽器之搜索列才可以用來進行搜尋。
- 若畫面中沒有明確能達成目標的元素，則優先推薦使用者開啟Google Chrome瀏覽器並進行問題相關的搜尋。
- 禁止回覆與HowTo3C相關的操作，若在畫面底部捕捉到與使用者目標相似的文字，則這是使用者在HowTo3C網頁輸入的問題，不要將其視為答案！
"""
...
def howto(req: https_fn.Request) -> https_fn.Response:
    if req.method == 'POST':
        try:
            if not req.content_type.startswith('multipart/form-data'):
                requests.post(statsurl, json={"events": [{"name": "howto400a","params": {}}]})
            ...
            if 'image' not in form:
                requests.post(statsurl, json={"events": [{"name": "howto400b","params": {}}]})
            ...
            if 'question' not in form:
                requests.post(statsurl, json={"events": [{"name": "howto400c","params": {}}]})
            ...
            if not isinstance(image_file, cgi.FieldStorage):
                requests.post(statsurl, json={"events": [{"name": "howto400d","params": {}}]})
            ...
            try:
                ...
                requests.post(statsurl, json={"events": [{"name": "howto400e","params": {}}]})
